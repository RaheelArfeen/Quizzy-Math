    ...
    return (
        <div className='min-h-screen bg-sky-50 font-fredoka p-4'>
            <motion.div
                ...
                className="max-w-6xl mx-auto py-8 flex items-center justify-center min-h-[calc(100vh-2rem)]"
            >
                <div className="bg-blue-100 rounded-3xl shadow-2xl p-4 md:p-8 w-full border-4 md:border-8 border-blue-300">
                    <div className="mb-6 md:mb-8">
                        <div className="flex flex-col sm:flex-row justify-between items-center mb-4 gap-2 md:gap-4">
                            <div className="text-center sm:text-left">
                                <h2 className="text-xl md:text-2xl lg:text-3xl font-bold text-purple-600">
                                    {kidName && <span className="text-blue-600">{kidName}'s </span>}
                                    Question {gameState.currentQuestion + 1} of {gameSettings.totalQuestions}
                                </h2>
                            </div>
                            <div className="text-lg md:text-xl lg:text-2xl font-bold text-blue-600 bg-white px-3 py-2 md:px-4 md:py-2 lg:px-6 lg:py-3 rounded-2xl shadow-inner">
                                Score: {gameState.score}
                            </div>
                        </div>
                        <div className="w-full bg-gray-100 rounded-full h-4 md:h-5 lg:h-6 border-4 border-gray-300">
                            <motion.div
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8 lg:gap-12 items-center">
                        <div className="text-center order-2 lg:order-1">
                            <motion.div
                                ...
                                className={`inline-flex items-center justify-center w-20 h-20 md:w-24 md:h-24 lg:w-28 lg:h-28 xl:w-32 xl:h-32 rounded-full text-white font-bold text-2xl md:text-3xl lg:text-4xl shadow-lg mb-6 md:mb-8 transition-colors duration-300 ${gameState.timeLeft <= 5 ? 'bg-orange-500' : 'bg-green-500'}`}
                            >
                                {gameState.timeLeft}
                            </motion.div>
                            ...
                            <div className='flex flex-col items-center'>
                                <AnimatePresence mode="wait">
                                    <motion.div
                                        ...
                                        className="text-2xl md:text-4xl lg:text-5xl xl:text-6xl font-bold text-purple-600 mb-4"
                                    >
                                        {gameState.questions[gameState.currentQuestion]?.question} = ?
                                    </motion.div>
                                </AnimatePresence>

                                <div className="mt-4 md:mt-6 h-12 md:h-16">
                                    <AnimatePresence>
                                        {gameState.showFeedback && (
                                            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
                                                {gameState.selectedAnswer === gameState.currentAnswer ? (
                                                    <div className="text-lg md:text-2xl lg:text-3xl font-bold text-green-600 animate-bounce flex items-center justify-center gap-2"><FaCheckCircle /> Correct! Great job!</div>
                                                ) : gameState.selectedAnswer === null ? (
                                                    <div className="text-lg md:text-2xl lg:text-3xl font-bold text-orange-600 flex items-center justify-center gap-1"><FaClock /> Time's up! Answer: {gameState.currentAnswer}</div>
                                                ) : (
                                                    <div className="text-lg md:text-2xl lg:text-3xl font-bold text-red-600 flex items-center justify-center gap-2"><FaRegTimesCircle /> Good try! Answer: {gameState.currentAnswer}</div>
                                                )}
                                            </motion.div>
                                        )}
                                    </AnimatePresence>
                        ...
                        <motion.div 
                            ...
                            initial="initial" 
                            animate="animate" 
                            className="grid grid-cols-2 gap-2 md:gap-3 lg:gap-4 order-1 lg:order-2"
                        >
                            {gameState.choices.map((choice, index) => (
                                <motion.button
                                    ...
                                    className={`font-bold py-3 md:py-4 lg:py-6 xl:py-8 px-3 md:px-4 lg:px-6 rounded-xl md:rounded-2xl lg:rounded-3xl text-lg md:text-xl lg:text-2xl xl:text-3xl transition-all duration-300 shadow-lg ${getChoiceButtonClass(choice)}`}
